...
const QuizLobbyPage = () => {
  ...
  const { data: playerScore, isLoading: isScoreLoading } = useGetPlayerScore(quizId);
  const hasPlayed = useMemo(() => playerScore !== undefined && Number(playerScore) > 0, [playerScore]);

  const { data: rawLeaderboardData, isLoading: isLeaderboardLoading, refetch: refetchLeaderboard } = useGetLeaderboard(quizId);
  const sortedLeaderboard = useMemo(() => { 
    if (!rawLeaderboardData || !rawLeaderboardData[0] || !rawLeaderboardData[1]) return [];
    const addresses = rawLeaderboardData[0] as `0x${string}`[];
    const scores = rawLeaderboardData[1] as bigint[];
    const combined = addresses.map((address, index) => ({ player: address, score: scores[index] }));
    return combined.sort((a, b) => Number(b.score) - Number(a.score));
  }, [rawLeaderboardData]);
  
  const { data: playFee, isLoading: isFeeLoading } = useGetPlayQuizFee();
  const { data: hasPaid, refetch: refetchPaidStatus } = useGetHasPaidStatus(quizId);
  ...
  const handleShare = () => {
    ...
    const twitterIntentUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(caption)}`;
    window.open(twitterIntentUrl, "_blank", "noopener,noreferrer");
  };

  if (isNaN(quizId)) {
    return <div className="text-center text-red-500 text-xl">Invalid Quiz ID.</div>;
  }

  if (isQuizLoading || isFeeLoading || isLeaderboardLoading || isScoreLoading) {
    return <div className="text-center text-primary text-xl">Loading Quiz Details...</div>;
  }
  ...
  if (isQuizError) {
    return (
      <div className="text-center text-red-500 bg-surface p-4 rounded-lg">
        ...
        <p className="text-xs font-mono mt-2 break-all">Error Details: {quizError?.message}</p>
      </div>
    );
  }

  if (!formattedQuizData || formattedQuizData.creator === "0x0000000000000000000000000000000000000000") {
    return <div className="text-center text-red-500 text-xl">Quiz not found on this contract.</div>;
  }
  ...
  const feeInEther = playFee !== undefined ? formatEther(playFee as bigint) : "...";

  return (
    <div className="container mx-auto">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        <div className="lg:col-span-2 bg-surface p-6 md:p-8 rounded-lg border border-border">
            <h1 className="text-3xl md:text-4xl font-bold text-primary mb-2 break-words">{formattedQuizData.title}</h1>
            <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-8 gap-4">
              ...
              >
                <svg viewBox="0 0 1200 1227" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" className="w-4 h-4" fill="currentColor">
            ...
            <div className="prose prose-invert text-gray-300 mb-10 max-w-none">
              <p className="whitespace-pre-wrap break-words">{formattedQuizData.description}</p>
            </div>
            ...
            {hasPaid ? (
                ...
                <div className="w-full text-center bg-gray-800 text-yellow-400 border border-yellow-500 rounded-lg p-4 flex items-center justify-center gap-3">
                    ...
                    <span className="font-semibold">Anda Sudah Mengerjakan Kuis Ini</span>
                </div>
                ) : (
                <Button
                ...
                >
                {isConfirming ? "Processing Payment..." : isPending ? "Waiting for Wallet..." : (
                    <div className="flex flex-col items-center">
                        <span>Play Quiz</span>
                        <span className="text-xs font-normal opacity-80 mt-1">Fee {feeInEther} STT</span>
                    </div>
                )}
                </Button>
            )}
